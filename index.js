...
function sostavChisla(massivChisel, chislo){
	const totalSum = massivChisel.reduce((acc, num) => (
		acc += num
	), 0);
	if ( totalSum < chislo ) {
		return [];
	...
	const workArray = massivChisel.filter((n) => n <= chislo);
	
	return findCombination(workArray, chislo);
}
...
function findCombination(arr, target) {
	...
	while ( localArray.length > 0 ) {
		const num = localArray.pop();
		if ( target - num === 0 ) {
			result.push([ num ]);
		}
		if ( target - num > 0 ) {
			const combinations = findCombination(localArray, target - num);
			if ( combinations.length > 0 ) {
				combinations.forEach((item) => result.push([
					...item,
					num,
				]));
			}
